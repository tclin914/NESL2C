...
//        printEXPBINDTUPLE(fptr,LHS,RHS);
  ...
  case NODE_SEQ_REF:{
    ...
    if(RHS->nodeType == NODE_INT){ // if RHS is simply a number
      ...
      case TypeSEQ:
        //TODO
        fprintf(fptr, "GET_ELEM_SEQ_(%s,%s,%d",node->string, LHS->string,RHS->iValue);
        fprintf(fptr, ");\n");
        break;
      default:
        assert(0);
      ...
      case TypeSEQ:
        //TODO
        fprintf(fptr, "GET_ELEM_SEQ_(%s,%s,%s",node->string, LHS->string,RHS->string);
        fprintf(fptr, ");\n");
        break;
      case TypeTuple:
        //TODO
  ...
  case NODE_NEW_SEQ:{
    ...
    case NODE_TOKEN:
      switch(LHS->valueType){
      case TypeSEQ:
        fprintf(fptr,"SET_ELEM_SEQ(%s,%s,%d);\n", 
                LHS->string, node->string, LHS->paramcount);
        break;
      case TypeTuple:
        fprintf(fptr, "SET_ELEM_PAIR(%s,%s,%d);\n",
  ...
  case NODE_APPLYBODY2:{
    ...
    case TypeSEQ:{
      int x=0;
      struct nodeType *loopme = LHS->typeNode;
